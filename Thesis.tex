...
\begin{definition}[Finite State Automata]
    Un automa a stati finiti (FSA) è una tupla A = $\langle \mathcal{S}, s\textsubscript0, \rightarrow, \mathcal{L} \rangle$ dove:
    % TODO complete itemize
    %\begin{itemize}
    %    \item $\matchcal{S}$ è un insieme finito di stati
    %    \item $s\textsubscript0 \in \mathcal{S}$ è lo stato iniziale dell'automa
    %    \item \rightarrow %(TODO add "function signature") è la funzione di transizione
    %\end{itemize}
\end{definition}

% TODO change s € S to correct notation
Tipicamente, oltre a quanto definito sopra, è solito trovare anche un insieme di stati di terminazione (o accettazione), detto F. Visto che in questo caso detto insieme non è stato definito assumiamo che ogni s € S sia uno stato di accettazione. Inoltre va notato che questa definizione coincide con quella di automa a stati finiti \emph{non deterministico}, solitamente indicato in letteratura con la sigla NFA (\emph{Non Deterministic Finite Automata}) e distinto dalla nozione di DFA (\emph{Deterministic Finite Automata}).
