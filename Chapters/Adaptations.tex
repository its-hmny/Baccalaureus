...
% Todo: Spiegare perche serve l'operazione di composizione (canali come interfaccie), esplicare l'algoritmo di composizione nostro (prodotto - ok) e sincronizzazione (la nostra versione), spiegare anche la tecninca di simulazione che pero ha dei grossi drawbacks, dare la 
...
\begin{algorithm}
    \caption{Algoritmo di sincronizzazione}
    \begin{algorithmic}
        \State $cFSA \gets createCompositionFSA(grSet)$ \Comment{Crea l'automa prodotto}
        \ForEach {$s \in cFSA.states$}
        \ForEach{$t_A \in s.transitions$}
            \If {$t_A.Kind == Send$}
            \State{$se \land \exists t_B \in  s.transitions. (t_B.Kind == Recv \land t_A.Channel == t_B.Channel)$}
            \State Aggiungi 
            \ElsIf{$t.Kind == Recv$}
            \ElsIf{$t.Kind == Spawn$}
            \Else{$Rimuovi la transizione t_A$}
            \EndIf
        \EndFor
        \EndFor
        % \While{$\exists$ gr $\in grSet$ non marcato} \Comment{Per ogni Goroutine trovata}
