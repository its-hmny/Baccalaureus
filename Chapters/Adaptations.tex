...
\lstinputlisting[language=Go, label={lst:DynamicAnalysis} caption={Codice sorgente per cui non è possibile calcolare $\mathcal{M}$ tramite analisi statica}, label={lst:DynamicAnalysis}]{Snippets/DynamicAnalysis.go}
\bigskip
...
\begin{figure}[h!]
    \centering
    \begin{tikzpicture}[->,>=stealth',node distance=4.2cm,scale=1, transform shape, initial text=$ $,]
        ...
        (2) edge[bend left] node[above,align=center]{$main \rightarrow worker_1$\\$main \rightarrow worker_2$} (3)
        (3) edge[bend left] node[below,align=center]{$worker_1 \rightarrow main$\\$worker_2 \rightarrow main$} (2);
    \end{tikzpicture}
