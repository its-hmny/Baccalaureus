...
\lstinputlisting[language=Go, caption={Codice sorgente per cui non e' possibile calcolare $\mathcal{M}$ tramite analisi statica}, label={lst:DynamicAnalysis}]{Snippets/DynamicAnalysis.go}
\bigskip
...
Una volta ottenuti tutti i metadati necessari sorge la necessita' di derivare dagli stessi le local views di ogni Gorountine creata durante l'esecuzione dell'intero programma, da queste poi potremmo procedere alla costruzione della global view.\bigskip\\
Fortunamente in questo fase l'esistenza di una gerarchia \emph{intrinseca} nelle Goroutine ci e' di aiuto: sappiamo che inizialmente esiste solo una Goroutine, quella che esegue il \texttt{main}, e sara' questa ad avviare le altre \emph{in cascata}.
