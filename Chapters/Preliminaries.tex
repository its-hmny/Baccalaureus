...
\begin{remark}
    È possibile trovare una definizione in cui non è presente l'insieme degli stati di terminazione (o di accettazione) $\mathcal{F}$. In tal caso assumiamo che ogni $s \in \mathcal{S}$ sia uno stato di accettazione.
\end{remark}
...
\end{remark}
Il seguente è un NFA N un grado di riconoscere la Regular Expression $(a|b)^*ba$:
\begin{figure}[h!] \label{fig:NonDeterministic_Example}
    \centering
...
\end{figure}\\
Si noti che questo è solo un \emph{possibile} NFA in grado di riconoscere il linguaggio dato ma ne esistono infiniti altri equivalenti ad esso.
...
L'automa presentato di seguito riconosce sempre la RegEx $(a|b)^*ba$ ed è del tutto equivalente a quello in figura \ref{fig:NonDeterministic_Example}, tuttavia è stato ottenuto proprio da quest'ultimo tramite l'algoritmo di \emph{Costruzione per sottoinsiemi}
\newpage
\begin{figure}[t!] \label{fig:Deterministic_Example}
    \centering
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \If{$\exists \alpha \in \mathcal{L}, \exists p, q \in \mathcal{S}$ tale che $(p,q)$ non è marcata $\land$ $\delta(p, \alpha) \neq \delta(q, \alpha)$}
...
\begin{figure}[ht] \label{fig:Minimization_Example}
    \centering
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,scale=1, transform shape, baseline=0]
...
\begin{figure}[ht] \label{fig:ChoreographyAutomata_Example}
    \centering
...
Ora che abbiamo introdotto tutti i concetti necessari possiamo definire di seguito l'operazione di \emph{Proiezione} su Choreography Automaton.
...
\end{definition}
\begin{remark}
    Si noti che esiste ed è possibile definire formalmente una funzione \emph{projection} che assegna ad ogni partecipante $p \in \mathcal{P}$ la sua relativa CFSM $M_p$.
\end{remark}
...
\begin{figure}[ht] \label{fig:Projection_Example}
    \centering
...
\begin{figure}[ht] \label{fig:Composition_Example}
    \centering
